...
for (page in c(1:25)) {
    print(page)            # 어느 코드에서 에러가 났는지 확인
    query <-sprintf('?page=%d&cate_cd=&srt=&searchKey=&keyWord=',page)
    url <- paste0(base_url, sub_url, query)
    html <-read_html(url)
    
    lis <- html %>% html_node('#container') %>% html_node('.new_book_list_wrap') %>% 
        html_node('.sub_book_list_area') %>% html_nodes('li')
    ...
    for (li in lis) {
        ...
        # 12페이지 두 개의 책에서 price 정보를 얻음
        if(length(ps)==0) {
            price <- 0
        } else {
            price <- ps[2] %>% html_node('.pbr') %>% html_node('strong') %>% html_text()
