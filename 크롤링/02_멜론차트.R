...
# 대부분의 사이트가 크롤링이 안되면 ua를 넣어주고 시작한다
...
table <- html %>% html_node('.service_list_song') %>% html_node('table')
trs <- table %>% html_node('tbody') %>% html_nodes('tr')
...
# 전주 순위 : 1(유지), 7(상승), 9(하락), 46(신규)
for (i in c(1,2,3,22)) {
    tr <- trs[i]                # trs[1], trs[7], trs[9], trs[46]
...
anchors <- tr %>% html_node('.wrap_song_info') %>% html_nodes('a')
