...
test[test%%3==0]               # 3의 배수, NA 선택
test[test%%3==0 & !is.na(test)]   # NA 제거한 3의 배수
...
df[df$gender=='F',]
# 25세 이상이고 남성인 행 추출
df[df$gender=='M' & df$age>=25,]   # 행 추출은 [x,]

# 2. if문
x <- 5
if(x%%2==0) {
   print('짝수입니다')     # indentation 주의할 것 & 반드시 해야할 것
} else {
   print('홀수입니다')
}

if (x>1e-10) {            # 0.0000000001 이렇게 표현해야 제대로된 음수, 양수 구분
    print('양수')
