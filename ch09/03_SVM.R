...
# 예측
svc_pred <- predict(svc, iris_test, type = 'class')

# 평가
table(svc_pred, iris_test$Species)
confusionMatrix(svc_pred, iris_test$Species)
...
svc001 <- svc100 <- svm(Species~.,iris_train, cost=0.01)
pred001 <- predict(svc001, iris_test, type = 'class')
table(pred001, iris_test$Species)

# 모델을 훈련했을 때의 데이터로 예측(좋은 결과가 나오지 않음)
