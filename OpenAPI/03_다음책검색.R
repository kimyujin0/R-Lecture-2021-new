...
# 결과가 list이기 때문에 매트릭스로 변환하여 저장
write.csv(as.matrix(df), 'OpenAPI/book.csv', fileEncoding = 'utf-8', row.names = F)

# 파일 내용중에 ','가 있어서 제대로 읽지 못해 sep를 \t로 변경
write.table(as.matrix(df), 'OpenAPI/book.csv', fileEncoding = 'utf-8', row.names = F, sep = '\t')

df2 <- read.csv('OpenAPI/book.csv', fileEncoding = 'utf-8', sep = '\t')
View(df2)
