...
s <- sd(weight)           # 여자 신생아의 몸무게 표준편차
n <- length(weight)
h0 <- 2800                # 모집단의 평균(기존 여아의 몸무게 평균-2800)
t.t <- (barx - h0) / (s / sqrt(n))   # 검정통계량

alpha <- 0.05             # 95% 신뢰수준
c.u <- qt(1-alpha, df=n-1)           # 누적 확률이 0.95
p.value <- 1 - pt(t.t, df=n-1)       # 검정통계량 2.23(t.t)보다 클 확률

# R에서 제공하는 t-test
t.test(weight, mu=2800, alternative="greater")      # alternative - 대립가설

# 도표 작성 
par(mar=c(0,1,1,1))
x <- seq(-3, 3, by=0.001)
y <- dt(x, df=n-1)    # x일 때 t값
plot(x, y, type="l", axes=F, ylim=c(-0.02, 0.38), 
     main="", xlab="t", ylab="")
abline(h=0) # x축을 그려라

polygon(c(c.u, x[x>c.u], 3), c(0, y[x>c.u], 0), col=2)   # =c.u
...
# 2. 모집단이 두개인 경우 - 독립인 두 집단, 대응을 이루는 두 집단
# 히스토그램
par(mar=c(2, 1, 1, 1))
...
# 정규성 테스트
shapiro.test(boy$weight)     # 정규성이 없다.
qqnorm(boy$weight)
qqline(boy$weight)
shapiro.test(girl$weight)    # 정규성을 가진다.
qqnorm(girl$weight)
qqline(girl$weight)
...
t.test(data$Prior, data$Post, paired=T, alternative="less") # paried - 서로 대응인 표본으로 인식
# 결론 - 영가설을 기각하므로 좋아졌다. 효과가 있다.