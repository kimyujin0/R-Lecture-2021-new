...
coef(model)              # y=0.428*x + 0.209*u + 1.262
...
# 잔차
fitted(model)         # 예측한 y 값
y
residuals(model)     # 잔차, y - fitted(y)
...
ny <- predict(model, newdata = new_data)
ny

s<-scatterplot3d(nx,nu,ny,
                 xlim=0:10, ylim = 7:23, zlim = 0:10,
                 pch = 20, type = 'h', col='red', angle = 60)
s$plane3d(model)
...
dim(trees)
summary(trees)
scatterplot3d(trees$Girth, trees$Height, trees$Volume)   # 맨 마지막을 종속변수로
